...
class RecommendFeedContainer extends StatefulWidget {
  const RecommendFeedContainer({super.key});

  @override
  State<RecommendFeedContainer> createState() => _RecommendFeedContainerState();
}

class _RecommendFeedContainerState extends State<RecommendFeedContainer> {
  ...
  Future<void> loadUserInfo(int userUid) async{
  ...
  Future<void> _loadFollowingInfo() async{
    try{
      ...
      List<User> fetchedAllUsersInfo = await LoadUserInfo.loadAllInfo();
      if(!mounted) return;

      print("Loading Follow Info....");
      users = List<User?>.filled(fetchedAllUsersInfo.length, null);

      print("Loading Follow Info....");
      for(int i = 0; i < fetchedAllUsersInfo.length; i++){
        await loadUserInfo(fetchedAllUsersInfo[i].user_uid ?? -5);
